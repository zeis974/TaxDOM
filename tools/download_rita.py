#!/usr/bin/env python3
"""
Download all RITA chapters (1-99) as XML files to tools/data/rita/xml/.
...
Output dir: tools/data/rita/xml/chapter_XX.xml (relative to project root)
"""
...
OUTPUT_DIR = PROJECT_ROOT / "tools" / "data" / "rita" / "xml"


def download_chapter_xml(session, chapter: int) -> bytes:
    data = {
...
def main() -> None:
    ...
    for chapter in chapters:
        ...
        if skip_existing and out_file.exists():
            ...
            to_fetch.append((chapter, out_file))
    ...
    if to_fetch:
        ...
        with FetcherSession(impersonate="chrome") as session:
            ...
            for chapter, out_file in to_fetch:
                print(f"  [dl]   chapter {chapter:02d} ...", end=" ", flush=True)
                try:
                    xml_bytes = download_chapter_xml(session, chapter)
